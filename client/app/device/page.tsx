...
const formatDeviceCode = (rawCode: string) => {
  let value = rawCode.toUpperCase().replace(/[^A-Z0-9]/g, "")
  if (value.length > 4) {
...
function DeviceAuthForm() {
  ...
  useEffect(() => {
    if (initialCode) {
      setUserCode(formatDeviceCode(initialCode))
    }
  }, [initialCode])
  ...
  const handleCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setUserCode(formatDeviceCode(e.target.value))
  }
    ...
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="animate-pulse flex flex-col items-center gap-4">
          <ShieldAlert className="w-8 h-8 text-yellow-300" />
          <p className="text-muted-foreground text-sm font-medium">Loading authorization page...</p>
        </div>
      </div>
    }>
      <DeviceAuthForm />
